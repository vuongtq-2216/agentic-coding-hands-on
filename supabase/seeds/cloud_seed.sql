-- ============================================================
-- Kudos Feature: Cloud Seed Data
-- ============================================================
-- Run this in Supabase Dashboard > SQL Editor
-- This creates fake auth users + profiles + kudos for demo
-- ============================================================

-- Step 0: Ensure tables exist (migration should have already run)
-- If not, run the migration first from supabase/migrations/

-- Step 1: Create fake auth users using Supabase's admin function
-- (This works in SQL Editor with service_role access)
DO $$
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE id = '11111111-1111-1111-1111-111111111111') THEN
    ...
    VALUES
      ('11111111-1111-1111-1111-111111111111', '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'huynhduong@sun-asterisk.com', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Huỳnh Dương Xuân Nhật"}', now(), now(), ''),
      ('22222222-2222-2222-2222-222222222222', '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'nguyenvan@sun-asterisk.com', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Nguyễn Văn Hoàng"}', now(), now(), ''),
      ('33333333-3333-3333-3333-333333333333', '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'tranthimai@sun-asterisk.com', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Trần Thị Mai"}', now(), now(), ''),
      ('44444444-4444-4444-4444-444444444444', '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'lequangminh@sun-asterisk.com', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Lê Quang Minh"}', now(), now(), ''),
      ('55555555-5555-5555-5555-555555555555', '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'phamthuhang@sun-asterisk.com', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Phạm Thu Hằng"}', now(), now(), ''),
      ('66666666-6666-6666-6666-666666666666', '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'vothanhson@sun-asterisk.com', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Võ Thanh Sơn"}', now(), now(), ''),
      ('77777777-7777-7777-7777-777777777777', '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'dangthilan@sun-asterisk.com', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Đặng Thị Lan"}', now(), now(), ''),
      ('88888888-8888-8888-8888-888888888888', '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'buiminhtuan@sun-asterisk.com', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Bùi Minh Tuấn"}', now(), now(), ''),
      ('99999999-9999-9999-9999-999999999999', '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'hoangthinga@sun-asterisk.com', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Hoàng Thị Nga"}', now(), now(), ''),
      ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'dothanhdat@sun-asterisk.com', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Đỗ Thành Đạt"}', now(), now(), '')
...
VALUES
  ...
  ('a0000001-0000-0000-0000-000000000001', '11111111-1111-1111-1111-111111111111', '22222222-2222-2222-2222-222222222222',
   ...
   'IDOL GIỚI TRẺ', ARRAY['#Dedicated', '#Inspiring'], ARRAY[]::TEXT[], true, 1000, now() - interval '2 hours'),
  ...
  ('a0000002-0000-0000-0000-000000000002', '33333333-3333-3333-3333-333333333333', '44444444-4444-4444-4444-444444444444',
   ...
   'LEADER CỦA NĂM', ARRAY['#Leadership', '#Teamwork', '#Dedication'], ARRAY[]::TEXT[], true, 856, now() - interval '5 hours'),
  ...
  ('a0000003-0000-0000-0000-000000000003', '55555555-5555-5555-5555-555555555555', '77777777-7777-7777-7777-777777777777',
   ...
   'IDOL GIỚI TRẺ', ARRAY['#Creative', '#Design', '#Excellence'], ARRAY[]::TEXT[], true, 723, now() - interval '8 hours'),
  ...
  ('a0000004-0000-0000-0000-000000000004', '66666666-6666-6666-6666-666666666666', '88888888-8888-8888-8888-888888888888',
   ...
   'HERO ĐÊM KHUYA', ARRAY['#Dedicated', '#Reliability', '#ProblemSolver'], ARRAY[]::TEXT[], true, 645, now() - interval '12 hours'),
  ...
  ('a0000005-0000-0000-0000-000000000005', '99999999-9999-9999-9999-999999999999', '11111111-1111-1111-1111-111111111111',
   ...
   'NGƯỜI TỔ CHỨC', ARRAY['#TeamBuilding', '#Fun', '#Connection'], ARRAY[]::TEXT[], true, 534, now() - interval '1 day'),
  ...
  ('a0000006-0000-0000-0000-000000000006', '22222222-2222-2222-2222-222222222222', '33333333-3333-3333-3333-333333333333',
   ...
   'HR TUYỆT VỜI', ARRAY['#HR', '#Onboarding', '#Support'], ARRAY[]::TEXT[], false, 234, now() - interval '1 day 3 hours'),
  ...
  ('a0000007-0000-0000-0000-000000000007', '44444444-4444-4444-4444-444444444444', '55555555-5555-5555-5555-555555555555',
   ...
   'MARKETING STAR', ARRAY['#Marketing', '#Growth', '#Achievement'], ARRAY[]::TEXT[], false, 189, now() - interval '1 day 6 hours'),
  ...
  ('a0000008-0000-0000-0000-000000000008', '77777777-7777-7777-7777-777777777777', '66666666-6666-6666-6666-666666666666',
   ...
   'CODE REVIEWER', ARRAY['#QA', '#CodeReview', '#Quality'], ARRAY[]::TEXT[], false, 167, now() - interval '2 days'),
  ...
  ('a0000009-0000-0000-0000-000000000009', '88888888-8888-8888-8888-888888888888', '99999999-9999-9999-9999-999999999999',
   ...
   'FINANCE HERO', ARRAY['#Finance', '#Transparency', '#Efficiency'], ARRAY[]::TEXT[], false, 145, now() - interval '2 days 4 hours'),
  ...
  ('a0000010-0000-0000-0000-000000000010', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '11111111-1111-1111-1111-111111111111',
   ...
   'KNOWLEDGE SHARER', ARRAY['#Learning', '#TechTalk', '#AI', '#Sharing'], ARRAY[]::TEXT[], false, 312, now() - interval '2 days 8 hours'),
  ...
  ('a0000011-0000-0000-0000-000000000011', '11111111-1111-1111-1111-111111111111', '44444444-4444-4444-4444-444444444444',
   ...
   'PROJECT HERO', ARRAY['#ProjectManagement', '#Adaptability'], ARRAY[]::TEXT[], false, 278, now() - interval '3 days'),
  ...
  ('a0000012-0000-0000-0000-000000000012', '33333333-3333-3333-3333-333333333333', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
   ...
   'SPEED DEMON', ARRAY['#Mobile', '#FastDelivery', '#Quality'], ARRAY[]::TEXT[], false, 201, now() - interval '3 days 5 hours'),
  ...
  ('a0000013-0000-0000-0000-000000000013', '55555555-5555-5555-5555-555555555555', '22222222-2222-2222-2222-222222222222',
   ...
   'DEBUG MASTER', ARRAY['#Debug', '#Payment', '#Teamwork'], ARRAY[]::TEXT[], false, 156, now() - interval '3 days 10 hours'),

  ('a0000014-0000-0000-0000-000000000014', '66666666-6666-6666-6666-666666666666', '77777777-7777-7777-7777-777777777777',
   'Design system mới mà chị Lan xây dựng giúp team dev tiết kiệm rất nhiều thời gian.',
   'DESIGN SYSTEM', ARRAY['#DesignSystem', '#Consistency', '#Productivity'], ARRAY[]::TEXT[], false, 189, now() - interval '4 days'),

  ('a0000015-0000-0000-0000-000000000015', '99999999-9999-9999-9999-999999999999', '33333333-3333-3333-3333-333333333333',
   'Chị Mai tổ chức workshop Communication Skills rất bổ ích!',
   'TRAINER', ARRAY['#Workshop', '#Communication', '#Learning'], ARRAY[]::TEXT[], false, 134, now() - interval '4 days 6 hours'),
  ...
  ('a0000016-0000-0000-0000-000000000016', '22222222-2222-2222-2222-222222222222', '88888888-8888-8888-8888-888888888888',
   ...
   'DEVOPS HERO', ARRAY['#DevOps', '#CICD', '#Automation'], ARRAY[]::TEXT[], false, 267, now() - interval '5 days'),

  ('a0000017-0000-0000-0000-000000000017', '44444444-4444-4444-4444-444444444444', '66666666-6666-6666-6666-666666666666',
   'Anh Sơn luôn test rất kỹ trước khi release. Bug rate giảm 80% so với quý trước!',
   'QUALITY GUARDIAN', ARRAY['#Testing', '#Quality', '#ZeroBug'], ARRAY[]::TEXT[], false, 198, now() - interval '5 days 8 hours'),
  ...
  ('a0000018-0000-0000-0000-000000000018', '77777777-7777-7777-7777-777777777777', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
   ...
   'UX INNOVATOR', ARRAY['#UX', '#Mobile', '#Innovation'], ARRAY[]::TEXT[], false, 145, now() - interval '6 days'),

  ('a0000019-0000-0000-0000-000000000019', '88888888-8888-8888-8888-888888888888', '55555555-5555-5555-5555-555555555555',
   'Hằng viết content marketing hay quá! Blog posts mang lại traffic organic tăng 150%.',
   'CONTENT CREATOR', ARRAY['#Content', '#Marketing', '#SEO'], ARRAY[]::TEXT[], false, 112, now() - interval '6 days 12 hours'),

  ('a0000020-0000-0000-0000-000000000020', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '99999999-9999-9999-9999-999999999999',
   'Chị Nga luôn xử lý expense report nhanh chóng và chính xác!',
   'FINANCE SUPPORT', ARRAY['#Finance', '#Support', '#Accuracy'], ARRAY[]::TEXT[], false, 98, now() - interval '7 days')
ON CONFLICT (id) DO NOTHING;

-- Step 4: Create kudos likes
